...
def _get_system_prompt() -> str:
    ...
    return (
        ...
        "4. 项目经历: At least 12 projects (3-4 per company). Each project MUST include:\n"
        "   - Project name (real, specific name)\n"
        "   - Time period (must span at least 6-12 months, not 1-3 months)\n"
        "   - 项目背景: project goal, business context, scale (users/traffic/data volume), key technical challenges\n"
        "   - 技术栈: core technologies, frameworks, middleware, tools used with brief rationale\n"
        "   - 遇到的问题与解决方案: 2-3 specific technical problems with detailed explanations:\n"
        "     * What was the problem (specific technical issue, error, bottleneck)\n"
        "     * How you investigated it (logs, metrics, debugging steps)\n"
        "     * Root cause analysis\n"
        "     * Implementation of the solution\n"
        "     * Measurable outcome (performance improvement, error reduction, etc.)\n"
        "   - 核心贡献: 6-8 bullet points starting with 主导/负责/参与, each with quantified outcome\n"
        ...
        "- Real company/university/project names only, NO placeholders like 某中型互联网公司\n"
        f"- Timeline consistent: projects within company tenure, no overlap, 8 years ending in {current_year}\n"
        "- Each project must span at least 6-12 months, not 1-3 months\n"
...
def _build_prompt(job_description: str) -> str:
    ...
    return (
        ...
        f"- Work entries: tech stack, responsibilities, decisions, problems/solutions, impact, quantified results\n"
        f"- Project entries (MUST have 12+ total):\n"
        f"  * Each project must span 6-12 months minimum (not 1-3 months)\n"
        f"  * Background: goal, context, scale, challenges\n"
        f"  * Tech stack with rationale\n"
        f"  * DETAILED PROBLEMS & SOLUTIONS (2-3 specific issues per project):\n"
        f"    - Specific technical problem/error/bottleneck\n"
        f"    - Investigation process (logs, metrics, debugging)\n"
        f"    - Root cause analysis\n"
        f"    - Solution implementation\n"
        f"    - Measurable outcome\n"
        f"  * 6-8 contribution bullets with quantified results\n"
        f"- Real Chinese university 50-150, real company/university/project names only (NO placeholders like 某中型互联网公司)\n"
